...
<table class="full-width mt-12 mb-10">
    ...
    <tr class="bg-grey">
        ...
        <!-- <th class="border-top-bottom text-left py-2">DETALLE</th> -->
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-left">
                ...
                <br/><small> {!! $row->item->detalle !!}
                
                @if($row->attributes)
                    @foreach($row->attributes as $attr)
                        <br/>{!! $attr->detalle !!} : {{ $attr->value }}
                    @endforeach
                @endif
                @if($row->discounts)
                    @foreach($row->discounts as $dtos)
                        <br/><small>{{ $dtos->factor * 100 }}% {{$dtos->detalle }}</small>
                    @endforeach
                @endif</small></td>
            </td>
    ...
    @endforeach
        ...
        @if($document->total_free > 0)
            <tr>                <td colspan="5" class="text-right font-bold">OP. GRATUITAS: {{ $document->currency_type->symbol }}</td>
