<template>
    <div class="card mb-0 pt-2 pt-md-0">
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <th class="font-weight-bold" style="width: 200px">Descripción</th>
                                                <th class="font-weight-bold"  style="width: 260px">Detalle</th>
                                        ...
                                        <tbody v-if="form.items.length > 0">
                                            <tr v-for="(row, index) in form.items">
                                                <td>{{ index + 1 }}</td>
                                                <td style="width: 200px">{{ row.item.description }}<br/><small>{{ row.affectation_igv_type.description }}</small></td>
                                                 <td style="text-center">{{ row.item.detalle }}</td>
        ...
        <servicio-form-item :showDialog.sync="showDialogAddItem"
                           :operation-type-id="form.operation_type_id"
                           :currency-type-id-active="form.currency_type_id"
                           :exchange-rate-sale="form.exchange_rate_sale"
                           :price_list_id="form.price_list_id"
                           @add="addRow"></servicio-form-item>
        ...
        <person-form :showDialog.sync="showDialogNewPerson"
         ...
         <servicio-options :showDialog.sync="showDialogOptions"
                          :recordId="documentNewId"
                          :showClose="false"></servicio-options> 
    </div>
</template>

<script>
    import ServicioFormItem from './partials/item.vue'
    import PersonForm from '../persons/form.vue'
    import DocumentOptions from '../servicio/partials/options.vue'
    ...
    export default {
        components: {ServicioFormItem, PersonForm, DocumentOptions, Logo},
        ...
        async created() {
            await this.initForm()
            await this.$http.get(`/${this.resource}/tablas`)
                .then(response => {
